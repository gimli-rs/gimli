...
#[derive(Debug, Clone)]
pub struct LineRows<R, Program, Offset = <R as Reader>::Offset, Address = <R as Reader>::Address>
where
    ...
    Offset: ReaderOffset,
{
    ...
    pub fn next_row(
        &mut self,
    ) -> Result<Option<(&LineProgramHeader<R, Offset>, &LineRow<R::Address>)>> {
...
pub type Opcode<R> = LineInstruction<R, <R as Reader>::Offset, <R as Reader>::Address>;

/// A parsed line number program instruction.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum LineInstruction<R, Offset = <R as Reader>::Offset, Address = <R as Reader>::Address>
where
    R: Reader<Offset = Offset>,
    Offset: ReaderOffset,
    Address: ReaderAddress,
{
    ...
    DefineFile(FileEntry<R, Offset, Address>),
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct LineRow<A: ReaderAddress = u64> {
    tombstone: bool,
    address: A,
...
impl<A: ReaderAddress> LineRow<A> {
        ...
        Ok(match instruction {
            ...
            LineInstruction::FixedAddPc(operand) => {
                if !self.tombstone {
                    ...
                    let offset = A::length(operand.into(), address_size)?;
                    self.address = self.address.add(offset, address_size)?;
                    self.op_index.0 = 0;
                }
                false
            }
            ...
            LineInstruction::SetAddress(address) => {
                self.tombstone = address == A::ones(program.header().encoding.address_size);
                if !self.tombstone {
    ...
    /// Perform any reset that was required after copying the previous row.
    #[inline]
    pub fn reset<R: Reader<Address = A>>(&mut self, header: &LineProgramHeader<R>) {
    ...
    fn apply_operation_advance<R: Reader<Address = A>>(
        &mut self,
        ...
        let address_advance = A::length(address_advance.0, address_size)?;
        self.address = self.address.add(address_advance, address_size)?;
        Ok(())
    }
    ...
    /// Section 6.2.5.1
    fn exec_special_opcode<R: Reader<Address = A>>(
        &mut self,
        opcode: u8,
        header: &LineProgramHeader<R>,
    ) -> Result<()> {
...
pub struct LineProgramHeader<R, Offset = <R as Reader>::Offset, Address = <R as Reader>::Address>
where
    R: Reader<Offset = Offset>,
    Offset: ReaderOffset,
    Address: ReaderAddress,
{
    ...
    include_directories: Vec<AttributeValue<R, Offset, Address>>,
    ...
    /// contexts."
    file_names: Vec<FileEntry<R, Offset, Address>>,
    ...
    comp_file: Option<FileEntry<R, Offset, Address>>,
}
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct IncompleteLineProgram<
    R,
...
> where
    ...
    Address: ReaderAddress,
{
    header: LineProgramHeader<R, Offset, Address>,
}
    ...
    pub fn sequences(self) -> Result<(CompleteLineProgram<R, Offset>, Vec<LineSequence<R>>)> {
        ...
        loop {
            ...
            sequences.push(LineSequence {
                ...
                // in a row.
                start: sequence_start_addr.unwrap_or_default(),
...
pub struct FileEntry<R, Offset = <R as Reader>::Offset, Address = <R as Reader>::Address>
where
    R: Reader<Offset = Offset>,
    Offset: ReaderOffset,
    Address: ReaderAddress,
{
    path_name: AttributeValue<R, Offset, Address>,
    directory_index: u64,
    timestamp: u64,
    size: u64,
    md5: [u8; 16],
    source: Option<AttributeValue<R, Offset, Address>>,
}
...
mod tests {
    ...
    fn test_line_rows_variance() {
        ...
        fn _f<'a: 'b, 'b, E: Endianity>(
            x: OneShotLineRows<EndianSlice<'a, E>>,
        ) -> OneShotLineRows<EndianSlice<'b, E>> {
            x
        }
    }

    /// Ensure that `LineInstruction<R>` is covariant wrt R.
    /// This only needs to compile.
    #[test]
    fn test_line_instruction_variance() {
        /// This only needs to compile.
        fn _f<'a: 'b, 'b, E: Endianity>(
            x: LineInstruction<EndianSlice<'a, E>>,
        ) -> LineInstruction<EndianSlice<'b, E>> {
            x
        }
    }
